...
export const buySubscription =catchAsyncError(async(req,res,next)=>{
 ...
 const plan_id= process.env.PLAN_ID||"plan_MLpTKKWipgZKKm"
...
res.redirect(`${process.env.FRONTEND_URL}/paymentsuccess?${razorpay_payment_id}`)
  ...
  export const cancelSubscription= catchAsyncError(async(req,res,next)=>{
   ...
   if(refundTime>gap){
      await instance.payments.refund(payment.rezorpay_payment_id);
      refund= true;
   }
   ...
   res.status(200).json({
      success: true,
      message:
      refund?"Subscription canceled,you will receive full refund within 7 days":
      "Subscription canceled, No refund initiated as subscription was canceled after 7 days"
   })
